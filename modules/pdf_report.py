...
class PDFReport(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 8, f"Generated by SpoofyVibe  |  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}", align="C")
    ...
    def _posture_badge(self, x, y, posture, size=14):
        """Draw a circular posture badge."""
        color = POSTURE_COLORS.get(posture, POSTURE_COLORS["?"])
        r = size / 2
        cx, cy = x + r, y + r
        ...
        # Posture text
        # Shrink to fit words like "Excellent"
        if len(posture) > 5:
            self.set_font("Helvetica", "B", int(size * 0.25))
        else:
            self.set_font("Helvetica", "B", int(size * 0.45))
            
        self.set_text_color(255, 255, 255)
        tw = self.get_string_width(posture)
        self.set_xy(cx - tw / 2, cy - size * 0.22)
        self.cell(tw, size * 0.45, posture)
...
def generate_pdf_report(results, filename="spoofyvibe_report.pdf"):
    ...
    pdf._section_title("Posture Distribution", y_pad=4)
    ...
    y = pdf.get_y() + 2
    posture_order = ["Excellent", "Good", "Moderate", "Weak", "Critical Risk"]
    bar_max_w = 100
    bar_h = 7
    ...
    for posture in posture_order:
        ...
        # Posture label
        self_font_size = 9 if len(posture) <= 5 else 7
        pdf.set_font("Helvetica", "B", self_font_size)
        pdf.set_text_color(*color)
        pdf.set_xy(14, y)
        pdf.cell(20, bar_h, posture, align="R")

        # Bar
        pct = count / domain_count if domain_count > 0 else 0
        fill_w = max(bar_max_w * pct, 2)
        pdf.set_fill_color(*color)
        pdf.rect(37, y + 1, fill_w, bar_h - 2, "F")

        # Count
        pdf.set_font("Helvetica", "", 8)
        pdf.set_text_color(*TEXT_PRIMARY)
        pdf.set_xy(37 + fill_w + 3, y)
    ...
    for result in results:
        ...
        # Posture badge
        pdf._posture_badge(18, y + 3, posture, size=12)
    ...
    for result in results:
        ...
        color = POSTURE_COLORS.get(posture, POSTURE_COLORS["?"])
        pdf.set_text_color(*color)
        pdf.cell(0, 7, _safe(f"{posture}  |  {domain}"), new_x="LMARGIN", new_y="NEXT")
